/**
 * DLC Portal Server with SBI Integration (Fixed Version)
 * 
...
// Lazy initialization of SBI integration
let sbiIntegration = null;

function getSBIIntegration() {
    if (!sbiIntegration) {
        console.log('Initializing SBI Integration...');
        const SBIIntegration = require('./sbi-integration');
        sbiIntegration = new SBIIntegration();
        console.log('SBI Integration initialized successfully');
    }
    return sbiIntegration;
}
...
app.post('/api/sbi/batch-id', async (req, res) => {
    try {
        ...
        // Initialize SBI integration if needed
        const sbi = getSBIIntegration();
        
        // Call SBI API
        const result = await sbi.getBatchId(state, date);
...
app.post('/api/sbi/verification-records', async (req, res) => {
    try {
        ...
        // Call SBI API
        const result = await sbi.fetchVerificationRecords(state, date, batchId);
...
app.post('/api/sbi/prepare-request', (req, res) => {
    try {
        ...
        const preparedRequest = sbi.prepareOutgoingRequest(eisPayload, txnType, txnSubType);
...
// Test SBI integration endpoint
app.get('/api/sbi/test', (req, res) => {
    try {
        const sbi = getSBIIntegration();
        res.json({
            success: true,
            message: 'SBI Integration is working',
            timestamp: new Date().toISOString()
        });
    } catch (error) {
        res.status(500).json({
...
app.listen(PORT, () => {
    ...
    console.log(`  GET http://localhost:${PORT}/api/sbi/test`);
});
