 ...
 * Use this for testing without SMS gateway
 * Copy this file to otpService.js to enable test mode
 */
...
class OTPService {
  constructor(dbPath = null) {
    ...
    // TEST MODE FLAG
    this.testMode = process.env.OTP_TEST_MODE === 'true' || true; // Set to true for testing
  ...
  async sendOTP(phoneNumber, ipAddress = null, userAgent = null) {
    try {
      ...
      if (this.testMode) {
        // TEST MODE: Don't send SMS, just log it
        console.log('\n' + '='.repeat(60));
        ...
        smsResult = {
          success: true,
          jobId: 'TEST_MODE_' + Date.now(),
          errorCode: '000',
          errorMessage: 'Test mode - SMS not sent'
        };
      } else {
        // PRODUCTION MODE: Send actual SMS
        const message = `Your OTP is ${otp}. Valid for 4 minutes. Do not share. -DLC Portal`;
        smsResult = await this.sendSMS(phoneNumber, message);
      ...
      const response = {
        success: true,
        message: this.testMode ? 'OTP generated successfully (TEST MODE - Check console)' : 'OTP sent successfully',
        expiresAt: expiredAt.toISOString(),
        jobId: smsResult.jobId
      };
      
      // In test mode, include OTP in response (NEVER do this in production!)
      if (this.testMode) {
