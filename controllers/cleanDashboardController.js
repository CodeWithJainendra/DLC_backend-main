...
class CleanDashboardController {
  /**
  ...
  static async getDashboardOverview(req, res) {
    try {
      ...
      const overallStats = await CleanDashboardController.getOverallStatistics(db);
      
      // Get top states by total pensioners
      const topStates = await CleanDashboardController.getTopStatesByPensioners(db);
      
      // Get verification method distribution
      const verificationMethods = await CleanDashboardController.getVerificationMethodDistribution(db);
      
      // Get biometric data distribution
      const biometricData = await CleanDashboardController.getBiometricDataDistribution(db);
      
      // Get success rates by top states
      const successRates = await CleanDashboardController.getSuccessRatesByState(db);
  ...
  static async getStateDashboardData(req, res) {
    try {
      ...
      const stateStats = await CleanDashboardController.getStateStatistics(db, stateCode);
      
      // Get verification method distribution for the state
      const verificationMethods = await CleanDashboardController.getStateVerificationMethods(db, stateCode);
      
      // Get biometric data distribution for the state
      const biometricData = await CleanDashboardController.getStateBiometricData(db, stateCode);
...
module.exports = CleanDashboardController;